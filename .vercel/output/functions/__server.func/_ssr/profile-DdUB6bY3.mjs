...
import { u as useAuth, A as AUTH_ACCOUNTS_KEY } from "./router-Bz0pR_Ai.mjs";
...
function readCurrentSecurityQuestion(email) {
  if (typeof window === "undefined") return "";
  try {
    const raw = localStorage.getItem(AUTH_ACCOUNTS_KEY);
    const accounts = raw ? JSON.parse(raw) : [];
    const match = accounts.find((account) => String(account.email ?? "").trim().toLowerCase() === email.trim().toLowerCase());
