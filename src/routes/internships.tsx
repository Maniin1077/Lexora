  ...
  head: () => ({
    meta: [
      { title: "Internships — Lexora Community" },
      {
        name: "description",
        content:
          "Explore internship opportunities, apply with one click, and track your application status.",
      },
    ],
  }),
...
function InternshipsPage() {
  ...
  return (
    <SiteLayout>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-20">
          ...
          <div className="mb-6 flex items-center justify-between rounded-xl border border-gold/40 bg-gold/5 p-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Add, edit, and remove internships including links and deadlines.
              </p>
            </div>
            <button
        ...
        {items === null ? (
          ...
          <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center">
            <p className="font-display text-xl text-primary">No internships yet</p>
            <p className="mt-2 text-sm text-muted-foreground">
              {isAdmin
                ? 'Click "New Internship" to publish an opportunity.'
      ...
      {editingInternship && isAdmin && user && (
        <InternshipDialog
  ...
  return (
    <article className="flex flex-col rounded-2xl border border-border bg-card p-6 transition hover:border-gold/40">
      ...
      <div className="mt-4 flex items-center gap-1 text-xs text-muted-foreground">
        ...
        <span className={closed ? "text-destructive" : "text-foreground"}>
          {new Date(internship.deadline).toLocaleString(undefined, {
            dateStyle: "medium",
            timeStyle: "short",
          })}
        </span>
...
function InternshipDialog({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Could not save internship", { description: message });
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <h2 className="font-display text-2xl text-primary">
          {isEditing ? "Edit Internship" : "New Internship"}
        </h2>
        <p className="mt-1 text-xs text-muted-foreground">
          Add details, registration link, and deadline for this opportunity.
        </p>
        ...
        <form onSubmit={submit} className="mt-5 space-y-3 text-sm">
          ...
          <Field
            ...
            onChange={(value) => setForm({ ...form, registrationLink: value })}
